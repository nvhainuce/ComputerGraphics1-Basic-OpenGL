...
//unbind VAO
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//khởi tạo ma trận chuyển trí (tranform)
		glm::mat4 transform = glm::mat4(1.0f);  ///tạo ma trận đơn vị
								//glm::translate(transform, glm::vec3(giá trị dịch theo x, giá trị dịch theo y, giá trị dịch theo z))
		transform = glm::translate(transform, glm::vec3(0.0f,  -0.5*(float)glfwGetTime() + Docaothem, 0.0f));  //transform=translate*transform
		transform = glm::rotate(transform, 10*(float)glfwGetTime() ,glm::vec3(0.0f, 0.0f, 1.0f));
		transform = glm::scale(transform, glm::vec3(0.3f, 0.3f, 0.3f));

		//tìm vị trí của uniform "TRans" trong vertex shader 
		GLuint transformLoc = glGetUniformLocation(ourShader.IDProgram, "Trans");
		//gán giá trị của uniform Trans = transform
